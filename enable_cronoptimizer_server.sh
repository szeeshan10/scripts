...
_note "Running Cron Optimizer enable operation for Server ID $server"

while read app; do
    _note "Enabling Cron Optimizer for App ID: $app on Server: $server"
    echo ""
    ...
    resp="$(curl -s -X POST \
        ...
        --header 'Authorization: Bearer '$accesstoken'' \
        --data 'server_id='$server'&app_id='$app'&status=enable' \
        'https://api.cloudways.com/api/v1/app/manage/cron_setting')"
...
_success "Cron Optimizer enabled successfully on all WordPress apps for Server ID $server."
