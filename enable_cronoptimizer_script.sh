...
for server in "${srvID[@]}"; do
    while read app ; do
        # Directly attempting to enable Cron Optimizer
        _note "Running for $server: $app"
        echo ""
        resp="$(curl -s -X POST --header 'Content-Type: application/x-www-form-urlencoded' --header 'Accept: application/json' --header 'Authorization: Bearer '$accesstoken'' --data 'server_id='$server'&app_id='$app'&status=enable' 'https://api.cloudways.com/api/v1/app/manage/cron_setting')"
